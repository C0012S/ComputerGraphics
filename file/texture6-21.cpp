#include <stdio.h>
#include <GL\glaux.h>	// 추가 종속성 : glaux.lib
#include <GL\glut.h>	// GL 폴더 추가

GLfloat mat_diffuse[] = { 0.25, 0.25, 1., 0. };	// 확산반사
GLfloat mat_specular[] = { 1., 1., 1., 0. };		// 경면반사
GLfloat light_position[] = { 10., 10., 20., 1. };  // 광원의 위치
GLfloat ref_plane[] = { 1.5, 1.5, 1.5, 0. };	// 텍스쳐 기준평면 // 1.5, 1.5, 1.5, 0.
...
void MyStripeImage() {					    // 텍스쳐 생성함수
	for (int j = 0; j < stripeImageWidth; j++) {
		stripeImage[4 * j] = (j < 8) ? 0 : 255;					// RED
		stripeImage[4 * j + 1] = 255 ;	// GREEN, 0~7 : 0, 8~31 : 255
		stripeImage[4 * j + 2] = (j < 16) ? 0 : 255;	// BLUE
		// 0~7 : (255,0,0,0), 8~31:(255,255,255,0)
		stripeImage[4 * j + 3] = 0;
	}
}
