...
struct debrisData {
	...
	float   color[3];
	float   scale[3]; //크기가 얼마나 변할지 //하나 하나의 particle마다 이 값을 가지고 있다
};
...
void newExplosion(void) {
	for (int i = 0; i < NUM_PARTICLES; i++) {
		...
		particles[i].color[0] = 1.0; // 1.0
		particles[i].color[1] = 1.0; // 1.0
		particles[i].color[2] = 0.5; // 0.5
	...
	for (int i = 0; i < NUM_DEBRIS; i++) {
		...
		debris[i].color[0] = 0.7;	// 0.7
		debris[i].color[1] = 0.7;	// 0.7
		debris[i].color[2] = 0.7;	// 0.7
...
void MyIdle(void) {
	if (fuel > 0) {
		for (int i = 0; i < NUM_PARTICLES; i++) {
			...
			particles[i].color[0] -= 1.0 / 500.0;	// 500.0
			if (particles[i].color[0] < 0.0) {
				particles[i].color[0] = 0.0;
			}

			particles[i].color[1] -= 1.0 / 100.0;	// 100.0
